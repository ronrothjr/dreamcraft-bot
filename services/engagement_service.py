    ...
    def player(self, args, channel, engagement, user):
        ...
        if not engagement:
            ...
            for char_name in args[2:]:
                ...
                messages.append(f'***{char.name}*** removed from _{engagement.name}_' if char else f'**{char_name}** was not found')
            ...
            for char_name in args[1:]:
                ...
                if char:
                    if str(char.id) in engagement.characters:
                        messages.append(f'***{char.name}*** is already in _{engagement.name}_')
                    else:
                        engagement.characters.append(str(char.id))
                        messages.append(f'Added ***{char.name}*** to _{engagement.name}_ engagement')
                else:
                    messages.append(f'***{char_name}*** not found. No character added to _{engagement.name}_')
            self.save(engagement, user)
            messages.append(engagement.get_string_characters(user))
            return messages
